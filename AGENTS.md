...
Trip planning agent swarm built entirely with **Codex agents and skills** — no external runtime or TypeScript needed. The orchestrator coordinates 9 specialist sub-agents that use WebSearch/WebFetch to research and plan comprehensive travel itineraries.
...
/trip-planner
```
Or ask naturally: "Plan a trip to Japan for 2 weeks in October" — Codex will automatically delegate to the trip-planner agent.
...
All research agents use Codex's built-in tools — no APIs or TypeScript required:
